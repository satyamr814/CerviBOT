...
def create_pipeline(X, feature_cols, y):
    ...
    print(f"\nClass imbalance ratio: {scale_pos_weight:.2f} (using scale_pos_weight)")
    
    # Create XGBoost model with class weighting
    xgb_params = {
        'n_estimators': 200,
        'max_depth': 6,
        'learning_rate': 0.05,
...
def train_model(csv_path, output_path=None):
    ...
    try:
        ...
        # Use calibrated model if it's better
        if auc_cal >= metrics['auc']:
            pipeline = calibrated_pipeline
            print("  Using calibrated model")
        else:
            print("  Using original model (calibration didn't improve)")
